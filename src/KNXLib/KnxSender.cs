...
internal abstract class KnxSender
{
    ...
    protected byte[] CreateRequestStatusDatagramCommon(string destinationAddress, byte[] datagram, int cemiStartPos)
    {
        var i = 0;

        datagram[cemiStartPos + i++] =
            KnxConnection.ActionMessageCode != 0x00
                ? KnxConnection.ActionMessageCode
                : (byte)0x11;

        datagram[cemiStartPos + i++] = 0x00;
        datagram[cemiStartPos + i++] = 0xAC;
        ...
        datagram[cemiStartPos + i++] = 0x00;
        var dstAddress = KnxHelper.GetAddress(destinationAddress);
        datagram[cemiStartPos + i++] = dstAddress[0];
        datagram[cemiStartPos + i++] = dstAddress[1];

        datagram[cemiStartPos + i++] = 0x01;
        datagram[cemiStartPos + i++] = 0x00;
        datagram[cemiStartPos + i] = 0x00;
