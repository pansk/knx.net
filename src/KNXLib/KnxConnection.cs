...
/// <summary>
///     Base class that controls the KNX connection, implemented by KnxConnectionRouting and KnxConnectionTunneling
/// </summary>
public abstract class KnxConnection
{
    ...
    internal void Event(string address, string state)
    {
        try
        {
            KnxEventDelegate?.Invoke(address, state);
        }
        catch (Exception e)
        {
            Logger.Error(ClassName, e);
    ...
    internal void Status(string address, string state)
    {
        try
        {
            KnxStatusDelegate?.Invoke(address, state);
        }
        catch (Exception e)
        {
            Logger.Error(ClassName, e);
