...
namespace KNXLib
{
    internal abstract class KnxReceiver
    {
        ...
        public void Stop()
        {
            try
            {
                if (_receiverThread?.ThreadState.Equals(ThreadState.Running) ?? false)
                {
            ...
            try
            {
                if (_consumerThread?.ThreadState.Equals(ThreadState.Running) ?? false)
                {
        ...
        protected void ProcessCemi(KnxDatagram datagram, byte[] cemi)
        {
            try
            {
                ...
                if (KnxConnection.Debug)
                {
                    ...
                    Logger.Debug(ClassName, "Event Aditional Info Length: " + datagram.AdditionalInfoLength);

                    if (datagram.AdditionalInfoLength > 0)
                        Logger.Debug(ClassName, "Event Aditional Info: 0x" + BitConverter.ToString(datagram.AdditionalInfo).Replace("-", string.Empty));
