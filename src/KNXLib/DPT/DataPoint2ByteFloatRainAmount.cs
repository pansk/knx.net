...
{
    internal sealed class DataPoint2ByteFloatRainAmount : IDataPoint
    {
        public string[] Ids
        {
            get
        ...
        public byte[] ToDataPoint(object val)
        {
            ...
            {
                Logger.Error("9.026", "input value received is not a valid type");
                return dataPoint;
            }

            if (value < -671088 || value > +670760)
            {
                Logger.Error("9.026", "input value received is not in a valid range");
                return dataPoint;
            }
